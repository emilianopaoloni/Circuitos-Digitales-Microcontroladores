...
DHT11_Status DHT11_Read(DHT11_Data* data) {
    ...
    //MCU envía señal de inicio (Start Signal)
    ...
    // Esperar respuesta del DHT11 (LOW por 80us)
    timeout_counter = 0;
    ...
    // Esperar a que el sensor suba la línea a HIGH (80us)
    timeout_counter = 0;
    while (!(DHT11_PIN & (1 << DHT11_BIT))) {
        _delay_us(1);
    ...
    // Esperar a que el sensor baje la línea de nuevo para empezar a transmitir
    timeout_counter = 0;
    while ((DHT11_PIN & (1 << DHT11_BIT))) {
        _delay_us(1);
        if (++timeout_counter > 100) return DHT11_ERROR_TIMEOUT;
